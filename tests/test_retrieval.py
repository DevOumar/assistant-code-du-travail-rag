...
def test_retrieve_maps_vector_results_to_retrieved_chunks(tmp_path: Path) -> None:
    ...
    with patch("retrieval.query_vector_database", return_value=_raw_result()) as query:
        chunks = retrieve("Quelle est la duree legale du travail ?", config=config)
...
def test_retrieve_cleans_parasitic_prefixes_before_search(tmp_path: Path) -> None:
    config = _config(tmp_path)

    with patch("retrieval.query_vector_database", return_value=_raw_result()) as query:
        retrieve("Peux-tu me dire, stp, quelle est la duree legale du travail ?", config=config)
...
def test_vector_store_retriever_matches_rag_pipeline_contract(tmp_path: Path) -> None:
    with patch("retrieval.query_vector_database", return_value=_raw_result()):
        chunks = VectorStoreRetriever(config=_config(tmp_path)).retrieve("duree travail", top_k=1)

    assert len(chunks) == 1
    assert chunks[0].text.startswith("La duree legale")
...
def test_retrieve_decomposed_can_use_hyde_expansion(tmp_path: Path) -> None:
    config = load_config(
        env_file=None,
        environ={
            "CHROMA_DB_DIR": str(tmp_path),
    ...
    with patch("retrieval.query_vector_database", side_effect=[first, second]) as query:
        with patch("retrieval.build_hyde_search_document", return_value="Hypothetical legal document"):
            chunks = retrieve_decomposed(
                "Quelle est la duree legale du travail ?",
                top_k=3,
                config=config,
    ...
    rendered = format_retrieved_chunks(
        [
            RetrievedChunk(
                chunk_id="chunk-1",
                text="Texte source",
                metadata={"article": "L3121-27", "section": "Duree", "source": "Legifrance"},
                distance=0.13,
                similarity=0.87,
            )
        ]
